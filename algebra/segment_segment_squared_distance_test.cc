...
TEST_CASE("segment_segment_squared_distance - line vs line") {
    REQUIRE(segment_segment_squared_distance<M>({0,0,0}, {5,0,0}, {3,3,8}, {3,-3,8}) == 64);
}

TEST_CASE("segment_segment_squared_distance - parallel") {
    V a(0,0,0), b(5,0,0), d(1,2,0);
    REQUIRE(segment_segment_squared_distance(a, b, a + d, b + d) == 4);
}

TEST_CASE("segment_segment_squared_distance - colinear") {
    REQUIRE(segment_segment_squared_distance<M>({0,0,0}, {5,0,0}, {7,0,0}, {10,0,0}) == 4);
}

TEST_CASE("segment_segment_squared_distance - identical") {
    REQUIRE(segment_segment_squared_distance<M>({0,0,0}, {5,0,0}, {0,0,0}, {5,0,0}) == 0);
}

TEST_CASE("segment_segment_squared_distance - overlap") {
    REQUIRE(segment_segment_squared_distance<M>({0,0,0}, {5,0,0}, {5,0,0}, {4,0,0}) == 0);
}

TEST_CASE("segment_segment_squared_distance - vertex vs vertex") {
    REQUIRE(segment_segment_squared_distance<M>({0,0,0}, {5,0,0}, {6,1,0}, {6,9,0}) == 2);
}

TEST_CASE("segment_segment_squared_distance - vertex vs line") {
    REQUIRE(segment_segment_squared_distance<M>({0,0,0}, {5,0,0}, {1,2,0}, {1,9,0}) == 4);
}
...
void REQUIRE_segment_segment_squared_distance(V a, V b, V c, V d, M e, const array<V, 4>& orig) {
...
void test_segment_segment_squared_distance(V a, V b, V c, V d, M e, const array<V, 4>& orig) {
