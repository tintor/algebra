...
TEST_CASE("segment_vs_segment_intersection_single_point - cross (diagonal)") {
    rational m, n;
    REQUIRE(segment_vs_segment_intersection_single_point<rational>({0, 0}, {9, 9}, {9, 0_q}, {0, 9}, &m, &n));
    REQUIRE(m == 1/2_q);
    REQUIRE(n == 1/2_q);
}
...
TEST_CASE("segment_vs_segment_squared_distance - identical") {
    Vec3<rational> a(0, 0, 0), b(5, 0, 0);
    REQUIRE(segment_segment_squared_distance(a, b, a, b) == 0);
}

TEST_CASE("segment_vs_segment_squared_distance - overlap") {
    Vec3<rational> a(0, 0, 0), b(5, 0, 0), c(4, 0, 0);
    REQUIRE(segment_segment_squared_distance(a, b, b, c) == 0);
}
